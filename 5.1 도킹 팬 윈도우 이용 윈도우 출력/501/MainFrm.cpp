...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (!m_paneSearch.Create(_T("검색"), this, CRect(0, 0, 200, 200), TRUE, 0xfafa,
		WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | WS_CLIPCHILDREN |
		CBRS_RIGHT | CBRS_FLOAT_MULTI)) {
		TRACE0("도킹팬(Pane)을 만들지 못했습니다.\n");
		return -1;	// 만들지 못했습니다.
	}
	m_paneSearch.SetMinSize(300);

	// 도킹할 수 있도록 함.
	m_paneSearch.EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_paneSearch);
